...
| **Branch changes document** | A read-only `DocumentPane` holding the active pane's branch diff, rendered by `BranchChangesRenderer` into the `branch-changes/` cache. `BranchChangesIdentity` (branch, base ref, repository) is its provenance; the cache file is regenerable storage, and the identity is what names the tab. |
