...
extension SessionPersistenceSerializationDomainTests {
    ...
    struct SessionPersistenceLoadTests {
        @Test("acknowledgement preserves a recovery warning raised by its catch-up save")
        func acknowledgementPreservesCatchUpRecoveryWarning() async throws {
            let directory = try TemporaryDirectory(prefix: "recovery-acknowledgement")
            let tempDir = directory.url
            ...
            try await SessionPersistence.withTemporarySupportDirectoryAsync(
                ...
                let originalWarning = try #require(result.recoveryWarning)
                let store = SessionStore(restoring: Self.snapshot(groupName: "current workspaces"))
                var warning: SessionPersistence.SessionRecoveryWarning? = originalWarning
                var failure: SessionPersistence.RecoverySnapshotReplacementError? = .warningNotActive
                let warningBinding = Binding(get: { warning }, set: { warning = $0 })
                let failureBinding = Binding(get: { failure }, set: { failure = $0 })

                SessionPersistence.restoreWorkspacesDidTurnOff()
                #expect(SessionPersistence.validateSnapshotForNewlyEnabledRestore() == nil)
                AwesoMuxApp.acknowledgeRecoveryWarning(
                    originalWarning,
                    thenSaving: store,
                    recoveryWarning: warningBinding,
                    failure: failureBinding
                )

                let activeWarning = try #require(SessionPersistence.activeRecoveryWarning)
                #expect(activeWarning.id != originalWarning.id)
                #expect(warning?.id == activeWarning.id)
                #expect(warning?.preventsInitialSave == true)
                ...
                #expect(
                    await waitUntil {
                        (try? SessionSnapshot.decode(from: Data(contentsOf: snapshotURL)))?.groups.map(\.name)
                            == ["current workspaces"]
                    })
            }
        }
