...
struct AgentIntegrationInstallerTests {
    ...
    private static func startExternalLockHolder(in directory: URL) async throws -> Process {
        ...
        // The sentinel is written only after flock succeeds, so a generous
        // ceiling adds no latency to passing runs but absorbs scheduling
        // stalls under full-suite load (GH #29).
        try #require(
