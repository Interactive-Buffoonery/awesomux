...
struct BridgeSurfaceCommandPolicyTests {
    ...
    func remoteGroupWithNoAttachCommandIsUnavailableNotLocalShell() {
        #expect(BridgeSurfaceCommandPolicy.command(
            bridgeEnabled: true, attachCommandAvailable: false, isRemote: true
        ) == .remoteUnavailable)
        // Local group unchanged: still falls back to a local shell.
        #expect(BridgeSurfaceCommandPolicy.command(
            bridgeEnabled: true, attachCommandAvailable: false, isRemote: false
        ) == .localShell)
        // Happy path unchanged.
        #expect(BridgeSurfaceCommandPolicy.command(
            bridgeEnabled: true, attachCommandAvailable: true, isRemote: true
        ) == .bridgeAttach)
    }
    ...
    func remoteGroupWithBridgeDisabledIsUnavailableNotLocalShell() {
        ...
        // `bridgeEnabled`.
        #expect(BridgeSurfaceCommandPolicy.command(
            bridgeEnabled: false, attachCommandAvailable: false, isRemote: true
        ) == .remoteUnavailable)
        // Even if some attach command were somehow available, a disabled
        // bridge can't use it for a remote group — still unavailable.
        #expect(BridgeSurfaceCommandPolicy.command(
            bridgeEnabled: false, attachCommandAvailable: true, isRemote: true
        ) == .remoteUnavailable)
    }
}
