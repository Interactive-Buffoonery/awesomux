...
struct AgentRuntimeEventProviderSessionIDTests {
    ...
    @Test("UUID session ids are canonicalized at the trust boundary")
    func uuidSessionIDIsCanonicalized() throws {
        let event = try #require(
    ...
    @Test("OpenCode preserves its opaque ses_ identity")
    func openCodeSessionIDSurvivesParsing() throws {
        let event = try #require(
            AgentRuntimeEvent.parse(
                data: Self.line(source: "opencode", providerSessionID: "ses_01JABCdef0123456789XYZ")
            )
        )

        #expect(event.providerSessionID == "ses_01JABCdef0123456789XYZ")
    }

    @Test(arguments: ["ses_", "msg_01JABC", "ses_bad/path", "ses_bad value"])
    func invalidOpenCodeSessionIDIsStripped(rawSessionID: String) throws {
        let event = try #require(
            AgentRuntimeEvent.parse(
                data: Self.line(source: "opencode", providerSessionID: rawSessionID)
            )
        )
    ...
    @Test(arguments: ["019c1234-5678-7abc-8def-0123456789ab", "project.branch_7"])
    func piSessionIDSurvivesParsing(sessionID: String) throws {
        let event = try #require(
            AgentRuntimeEvent.parse(data: Self.line(source: "pi", providerSessionID: sessionID))
        )

        #expect(event.providerSessionID == sessionID)
    }

    @Test(arguments: ["-leading", "trailing-", "bad/path", "bad value"])
    func invalidPiSessionIDIsStripped(rawSessionID: String) throws {
        let event = try #require(
            AgentRuntimeEvent.parse(
                data: Self.line(source: "pi", providerSessionID: rawSessionID)
            )
        )
