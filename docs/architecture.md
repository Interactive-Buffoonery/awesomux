...
- **Recovery write gate:** a load that quarantines, conflicts, or sanitizes-without-a-safe-archive protects the file on disk from automatic writes until the user resolves it. Any write refused at quit is parked beside the live file as `session-state.unsaved-<timestamp>-<id>.json` rather than dropped — the newest of those is the session that was open when the app last closed, and renaming it to `session-state.json` (with the app quit) restores it. All four quarantine families (`corrupted-`, `sanitized-`, `conflict-`, `unsaved-`) are owner-only and retained ten deep per family, oldest evicted first.
