...
- **Format:** JSON via `JSONEncoder` / `SessionSnapshot`; writes coalesced in fixed 500 ms windows to avoid thrashing without postponing checkpoints during continuous changes.
...
- **Recovery write gate:** a load that quarantines, conflicts, or sanitizes-without-a-safe-archive protects the file on disk from automatic writes until the user resolves it. A write refused at quit is parked beside the live file as `session-state.unsaved-<timestamp>-<id>.json` rather than dropped, best-effort — it is skipped for an empty session, and it cannot beat the size cap that refused the live write in the first place. Termination waits at most two seconds for an approved replacement. On timeout it performs no second write to the same slow directory; the protected prior file remains unless the already-in-flight replacement finishes before process exit. Gate refusals and ordinary write failures still use the archive fallback. A successful quit adds no archive, and the newest one can be older than `session-state.json`: compare timestamps before recovering one, and copy rather than rename. All four quarantine families (`corrupted-`, `sanitized-`, `conflict-`, `unsaved-`) are owner-only and retained ten deep per family. Load-time families evict oldest-first; the quit-time family pins the first capture of the current incident until a successful live save, captured after the incident began, resolves its owner-only incident marker on disk.
- **Unvalidated snapshots:** launching with restore disabled never reads the file, so the write path validates it lazily before the session's first write instead of trusting that a load already happened. Anchoring that check to the settings toggle would miss writers that are not co-located with the view observing it.
