...
- **Execution-plan migration:** pane plans are additive and do not bump the snapshot schema. A missing pane plan inherits the owning group's legacy `RemoteTarget` during restore; a present plan is authoritative. Malformed active pane plans fail decoding and trigger the normal archive-and-reset path, while malformed recently-closed rows remain isolated to that disposable row.
