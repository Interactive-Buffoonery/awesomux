...
public enum AgentTranscriptRenderer {
    ...
    ///
    /// ponytail: one fixed window rather than a chunked backwards walk that keeps
    ...
    public struct Chrome: Sendable {
        ...
        /// Sentence shown when the session was matched by working directory
        /// rather than by the id the agent reported — a directory match is a
        /// guess, and the document has to say so. Nil when the id was reported.
        public var provenanceNotice: String?
        ...
        public init(
            ...
            self.provenanceNotice = provenanceNotice
        }
    ...
    static func render(
        ...
        let provenanceNotice = provenanceNotice(chrome)
        ...
        let reserved =
            header.utf8.count + provenanceNotice.utf8.count + truncationNotice.utf8.count
            + emptyWindowNotice.utf8.count
    ...
    static func provenanceNotice(_ chrome: Chrome) -> String {
        chrome.provenanceNotice.map(italicNotice) ?? ""
    }
