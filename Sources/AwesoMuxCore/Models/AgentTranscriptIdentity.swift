...
public struct AgentTranscriptIdentity: Hashable, Sendable {
    ...
    /// A UUID string. Whitespace-trimmed at construction.
    public let sessionID: String

    /// Returns `nil` unless `agentKind` is a provider whose transcript layout
    /// awesoMux knows and `sessionID` is a UUID.
    ///
    ...
    /// the set that can be stored as provenance cannot drift apart. The UUID
    /// gate is deliberately stricter than
    /// `AgentRuntimeSource.validatedProviderSessionID`, which exempts Grok — a
    /// Grok id must never reach a path or a staged command.
    public init?(agentKind: AgentKind, sessionID: String) {
        guard AgentTranscriptImporter.Provider(agentKind: agentKind) != nil else {
            return nil
        }
    ...
    public init(from decoder: Decoder) throws {
        ...
        guard
            ...
            throw DecodingError.dataCorruptedError(
                forKey: .sessionID,
                in: container,
                debugDescription: "An agent transcript identity requires a supported provider and a UUID session id."
            )
        }
        self = identity
    }
